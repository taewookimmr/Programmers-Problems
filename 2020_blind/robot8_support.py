...
def solution(board):
    ...
    def simple_search():
        world=[[e for e in row] for row in board]
        world[0][0] = infinite
        world[0][1] = infinite
        q = [] 
        q.append([0,0])
        ...
        while len(q):
            ...
            for j in range(n):
                ...
                if world[r][c]<=-1:
                    continue
                if r==lim-1 and c == lim-1:
                    print(world)
                    return answer
                world[r][c] = -2
                
                if r+1 < lim and world[r+1][c]==infinite:
                    q.append([r+1,c])
                    
                if c+1 < lim and world[r][c+1]==infinite:
                    q.append([r,c+1])      
        
                if r-1 >= 0 and world[r-1][c]==infinite:
                    q.append([r-1,c])
        
                if c-1 >= 0 and world[r][c-1]==infinite:
                    q.append([r,c-1])
