...
def solution(board):
    ...
    def renewal(pos, distance):
        # 움직인 후의 좌표
        r1,c1,r2,c2=pos
        prev1, prev2 = board[r1][c1], board[r2][c2]
    ...
    def move_trans(pos, distance, move): 
        ...
        if check_trans([r1,c1,r2,c2]):
            if renewal([r1,c1,r2,c2], distance+1):
                queue.append([[r1,c1,r2,c2], distance+1, move])
        ...
        if axis == 1: ## axis = number1
            ...
            if board[checkr][checkc]<0 or board[dstr][dstc]<0:
                ...
                if renewal([r1, c1, dstr, dstc], distance+1):
                    move = int(axis*(-1) + ccw *(-0.5) + 5.5)
                    queue.append([[r1, c1, dstr, dstc], distance+1, move])
            ...
            if board[checkr][checkc]<0 or board[dstr][dstc] <0:
                ...
                if renewal([dstr, dstc, r2, c2], distance+1):
                    move = int(axis*(-1) + ccw *(-0.5) + 5.5)
                    queue.append([[dstr, dstc, r2, c2], distance+1, move])
        

    distance = 0
    queue.append([p, distance, -1]) # -1은 prevact에 해당함, 즉 아직 아무것도 안했다는 뜻 

    while len(queue):

        print(board)
        print(len(queue))
        pos, distance, prevact = queue.pop()
        print(pos)
        ...
        for axis in [1,-1]:
            for ccw in [1,-1]:
                move = int(axis*(-1)+ccw*(-0.5)+5.5)
                counter = 13-(move if move>5 else move+4)
                if counter != prevact:
